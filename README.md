...
<i>Author</i>: Paul Michel, Tatsunori Hashimoto, Graham Neubig<br>
<i>Comments</i>:<br>
继续上周对DRO topic的讨论。这篇paper是Michel对自己去年ICLR 2021工作P-DRO的一个优化，他在P-DRO基础上做了关键性的改进提出这篇RP-DRO（Ratio-based P-DRO）。DRO的Loss换个角度想救是在优化一个Worst-case Loss，而ERM是在优化Average Loss。这篇paper不直接去建模Worst-case distribution了，而是建模training distribution和Worst-case distribution之间的likelihood ratio。具体来说改进有三点：1. 对likelihood ratio加上一个normalization constraint（mini-batch level）；2. 将之前约束maximizer和真实分布之间距离的KL散度写成一个惩罚项；3. 使用simultaneous gradient update同时更新minimizer和maximizer。

对P-DRO里由于importance sampling产生的概率之比这个项可以写成一个新的uncertainty set。这样做的好处是给生成模型q提供了更多选择，但是同时要加上normalization constraint，不然的话maximizer可以无限制的调大分配给每个sample的weight从而minimizer就无从优化。之前的方法把这个ratio在每步gradient update时投影回去，这篇paper提出两种normalization constraints，分别是self-normalization（把mini batch中所有ratio做平方和）以及batch-level normalization（在每个mini batch用softmax的方式做归一化）。实验发现第二种方法不需要引入新的hyper parameter，且在训练时更稳定，因此作者推荐第二种。
